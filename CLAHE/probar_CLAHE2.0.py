"""
SCRIPT: probar_CLAHE2.0.py

Side-by-side test bench for the CLAHE enhancement pipeline.

Reads a video file, runs each frame through aplicar_CLAHE() (from
metodo_CLAHE.py), and displays the result alongside the untouched
original so they can be compared visually and quantitatively. All metric
functions are imported from the metricas_ofc/ package. Mirror of
RETINEX/probar_retinex4.0.py — same display flow, same CSV output
structure, same helper functions. Only the enhancement step differs.
...
APLICAR_SOLO_EN_ROI:  if True, CLAHE is only applied inside the face
                      ...
                      If False, CLAHE is applied to the entire frame.
...
while True:
    ...
    if APLICAR_SOLO_EN_ROI:
        ...
        if bbox is not None:
            ...
            if roi.size > 0:
                frame_mejorado[y:y+hh, x:x+ww] = aplicar_CLAHE(roi)
    else:
        frame_mejorado = aplicar_CLAHE(frame_original)
    ...
    cv2.imshow("CLAHE  --  enhanced (left)  vs  original (right)", combinado)
